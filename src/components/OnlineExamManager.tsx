...
export const OnlineExamManager: React.FC<OnlineExamManagerProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'extra_exam' && (
        <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border-2 border-amber-300 dark:border-amber-500/30 text-xs text-amber-950 dark:text-amber-200 flex items-start gap-3 shadow-xs">
          <AlertCircle className="w-4 h-4 shrink-0 mt-0.5 text-amber-800 dark:text-amber-400" />
          <div className="text-amber-950 dark:text-amber-200 leading-relaxed">
            <strong className="font-extrabold text-amber-950 dark:text-amber-100">નોંધ (Extra Exams):</strong> સામાન્ય જ્ઞાન (GK), ક્વિઝ કે સ્પર્ધાત્મક કસોટીઓ સત્તાવાર શૈક્ષણિક પ્રગતિપત્રક (Official Marks/Results) થી સંપૂર્ણપણે સ્વતંત્ર છે. તમે કોઈપણ સમયે આ પરીક્ષાઓનું સંચાલન અથવા સફાઈ કરી શકો છો.
          </div>
        </div>
      )}
              ...
              >
                ...
                <div>
                  <div className="flex items-center justify-between gap-2 mb-3">
                    ...
                    {/* Status Badge */}
                    {isLive ? (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 dark:bg-emerald-500/20 text-emerald-950 dark:text-emerald-300 border border-emerald-400 dark:border-emerald-500/40 flex items-center gap-1.5 animate-pulse">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-600 dark:bg-emerald-400" />
                        <span className="text-emerald-950 dark:text-emerald-200 font-extrabold">લાઈવ (Live)</span>
                      </span>
                    ) : exam.status === 'scheduled' ? (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 dark:bg-blue-500/20 text-blue-950 dark:text-blue-300 border border-blue-400 dark:border-blue-500/30 flex items-center gap-1">
                        <Clock className="w-3 h-3 text-blue-700 dark:text-blue-300" />
                        <span className="text-blue-950 dark:text-blue-200 font-extrabold">શેડ્યૂલ</span>
                      </span>
                    ) : exam.status === 'completed' ? (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-purple-100 dark:bg-purple-500/20 text-purple-950 dark:text-purple-300 border border-purple-400 dark:border-purple-500/30">
                        <span className="text-purple-950 dark:text-purple-200 font-extrabold">પૂર્ણ (Completed)</span>
                      </span>
                    ) : (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 dark:bg-slate-500/20 text-slate-800 dark:text-slate-300 border border-slate-300 dark:border-slate-500/30">
                        <span className="text-slate-900 dark:text-slate-200 font-extrabold">ડ્રાફ્ટ (Draft)</span>
                      </span>
                    )}
                  </div>
