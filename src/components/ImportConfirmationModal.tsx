    ...
    <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-3 sm:p-4 backdrop-blur-sm overflow-y-auto animate-fadeIn">
      <div className="bg-slate-900 rounded-2xl border border-slate-700 shadow-2xl max-w-4xl w-full max-h-[92vh] flex flex-col text-white my-auto overflow-hidden">
        ...
        <div className="p-4 bg-slate-950/60 border-b border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="p-3 rounded-xl border bg-slate-900/80 border-slate-800">
            <div className="flex items-center justify-between mb-1">
        ...
        <div className="px-4 pt-3 pb-2 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900">
          ...
          <div className="flex items-center gap-2">
            {activeTab === 'updates' && updatedEntries.length > 0 && (
              <label className="flex items-center gap-1.5 text-xs text-slate-300 cursor-pointer select-none bg-slate-800 px-2.5 py-1.5 rounded-lg border border-slate-700">
                <input
                  type="checkbox"
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-40 sm:w-56 bg-slate-950 border border-slate-700 rounded-lg pl-8 pr-3 py-1 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500"
              />
            </div>
          </div>
        </div>
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 rounded-xl border transition-all ${
                          hasChanges
                            ? 'bg-slate-950/80 border-blue-900/80 hover:border-blue-700/80'
                      ...
                      >
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 pb-2 border-b border-slate-800/80">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {item.grNumber && (
                              <span className="px-2 py-0.5 rounded bg-slate-800 text-amber-300 font-mono text-[11px] border border-slate-700 font-semibold">
                                G.R. #{item.grNumber}
                              </span>
                            )}
                            <span className="px-2 py-0.5 rounded bg-emerald-950 text-emerald-300 text-[11px] border border-emerald-800">
                          ...
                          <div>
                            {hasChanges ? (
                              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-blue-950 text-blue-300 border border-blue-700 inline-flex items-center gap-1">
                          ...
                          {hasChanges ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                              {item.changes!.map((ch, cIdx) => (
                                <div
                                  key={cIdx}
                                  className="p-2 rounded-lg bg-slate-900/90 border border-blue-900/50 text-[11px]"
                                >
                                  <div className="text-slate-400 font-medium mb-1 flex items-center justify-between">
                                    <span>{ch.fieldLabel}</span>
                                    <span className="text-[10px] text-blue-400 font-mono">ફેરફાર</span>
                                  </div>
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <span className="line-through text-red-400/80 bg-red-950/40 px-1.5 py-0.5 rounded border border-red-900/40">
                                      {ch.oldValue || '(ખાલી)'}
                                    </span>
                                    <ArrowRight className="w-3 h-3 text-blue-400 shrink-0" />
                                    <span className="font-bold text-emerald-300 bg-emerald-950/60 px-1.5 py-0.5 rounded border border-emerald-800">
                                      {ch.newValue}
                                    </span>
                                  </div>
                                </div>
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {filteredNew.map((s, idx) => (
                        <tr key={idx} className="hover:bg-slate-850/50">
                          ...
                          <td className="p-2.5 font-mono text-slate-300">{s.rollNumber || '-'}</td>
                          <td className="p-2.5">
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/80 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            <button
              ...
              disabled={isSubmitting}
              onClick={onClose}
              className="px-4 py-2 rounded-xl bg-slate-800 text-slate-300 text-xs font-semibold hover:bg-slate-700 transition-colors disabled:opacity-50"
            >
              રદ કરો
            </button>
            <button
